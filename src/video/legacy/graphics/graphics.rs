...
impl Graphics {
    ...
    pub fn fill_rect(&mut self, rect: &Rectangle) {
        let p0 = &rect.p0;
        self.device
            ._vga13h_fill_rect(p0.x, p0.y, rect.width, rect.height, self.color.as_u8());
    }

    pub fn draw_rect(&mut self, rect: &Rectangle) {
        let p0 = &rect.p0;
        self.device
            ._vga13h_draw_rect(p0.x, p0.y, rect.width, rect.height, self.color.as_u8());
    }

    pub fn fill_triangle(&mut self, p0: &PointUnsigned, p1: &PointUnsigned, p2: &PointUnsigned) {
        self.device
            ._vga13h_fill_triangle(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, self.color.as_u8());
    }

    pub fn draw_triangle(&mut self, p0: &PointUnsigned, p1: &PointUnsigned, p2: &PointUnsigned) {
        self.device
            ._vga13h_draw_triangle(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, self.color.as_u8());
    }

    pub fn draw_line(&mut self, p0: &PointUnsigned, p1: &PointUnsigned) {
        self.device
            ._vga13h_draw_line(p0.x, p0.y, p1.x, p1.y, self.color.as_u8());
    }

    pub fn draw_bitmap(&mut self, p: &PointUnsigned, bitmap: &Bitmap) {
        self.device
            ._vga13h_draw_bitmap(p.x, p.y, bitmap.width, bitmap.height, &bitmap.mem);
    }

    pub fn draw_char(&mut self, p: &PointUnsigned, char: char) {
        self.device
            ._vga13h_draw_char_transparent(p.x, p.y, char, &self.font, self.color.as_u8())
    }

    pub fn draw_str(&mut self, p: &PointUnsigned, str: &str) {
        self.device
            ._vga13h_draw_string(p.x, p.y, str, &self.font, self.color.as_u8())
    }

    pub fn fill_elipse(&mut self, p: &PointUnsigned, width: usize, height: usize) {
        self.device
            ._vga13h_fill_ellipse(p.x, p.y, width, height, self.color.as_u8());
    }

    pub fn draw_elipse(&mut self, p: &PointUnsigned, width: usize, height: usize) {
        self.device
            ._vga13h_draw_ellipse(p.x, p.y, width, height, self.color.as_u8());
    }
