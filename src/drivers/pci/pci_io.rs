...
impl PciDevice {
    pub fn pci_read8(&self, reg: u32) -> u8 {
        if let Some(ptr) = self.get_mmio_addr(reg) {
            ...
            unsafe {
                outl(PCI_CONFIG_ADDR, self.pci_addr(reg));
                inb(PCI_CONFIG_DATA + (reg & 0x03) as u16)
            }
        }
    }

    pub fn pci_read16(&self, reg: u32) -> u16 {
        if let Some(ptr) = self.get_mmio_addr(reg) {
    ...
    pub fn pci_read32(&self, reg: u32) -> u32 {
        if let Some(ptr) = self.get_mmio_addr(reg) {
    ...
    pub fn pci_write8(&self, reg: u32, data: u8) {
        if let Some(ptr) = self.get_mmio_addr(reg) {
    ...
    pub fn pci_write16(&self, reg: u32, data: u16) {
        if let Some(ptr) = self.get_mmio_addr(reg) {
    ...
    pub fn pci_write32(&self, reg: u32, data: u32) {
        if let Some(ptr) = self.get_mmio_addr(reg) {
    ...
    fn get_mmio_addr(&self, reg: u32) -> Option<*mut u8> {
        let id = self.base_id();
    ...
    #[inline(always)]
    fn pci_addr(&self, reg: u32) -> u32 {
        0x8000_0000 | self.base_id() | (reg & 0xfc)
    }
}
