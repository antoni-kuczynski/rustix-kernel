...
use crate::boot::cpuid::{CPU_ID, CpuId};
...
impl IoRemapManager {
    ...
    unsafe fn ioremap(
        ...
        assert!(
            ...
            "ioremap align must be power of two"
        );
        ...
        if !vmm_map_range_ext(
            ...
            panic!(
                " [IOREMAP] Mapping at phys {:#011x}, size {}, align {} at virt {:#011x} failed.",
...
pub fn ioremap_permanent(phys_addr: PhysAddr, size: u64, align: usize) -> IoAlloc {
    unsafe { IOREMAP_MANAGER.lock().ioremap(phys_addr, size, align, 0) }
}
...
pub fn ioremap_ext_permanent(phys_addr: PhysAddr, size: u64, align: usize, flags: u64) {
    unsafe {
        IOREMAP_MANAGER
            .lock()
            .ioremap(phys_addr, size, align, flags);
    }
}
