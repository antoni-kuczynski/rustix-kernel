...
impl PageManager {
    ...
    pub unsafe fn eba_map_page(&self, virt: VirtAddr, phys: PhysAddr, page_size: &PageSize, alloc_in_pmm: bool) {
    ...
    pub unsafe fn eba_map_range(&self, virt_start: VirtAddr, phys_start: PhysAddr, length: u64, page_size: &PageSize, alloc_in_pmm: bool) {
        ...
        while mapped_bytes < length {
            self.eba_map_page(VirtAddr::new_truncate(current_virt), PhysAddr::new_truncate(current_phys), page_size, alloc_in_pmm);
            current_virt += step;
            current_phys += step;
            mapped_bytes += step;
        }
    ...
    pub unsafe fn early_unmap_page(&self, virt: VirtAddr, phys: PhysAddr, page_size: &PageSize, free_in_pmm: bool) {
        let indexes = PageIndexes::get_from_virt(virt);
        let pml4 = PageTable::from_cr3();
        let pdpt3 = (*pml4).get_ptr_from_index_or_eba_kmalloc(indexes.pml4_index());
    ...
    pub unsafe fn early_unmap_range(&self, virt_start: VirtAddr, phys_start: PhysAddr, length: u64, page_size: &PageSize, free_in_pmm: bool) {
        ...
        while unmapped_bytes < length {
            self.early_unmap_page(VirtAddr::new_truncate(current_virt), PhysAddr::new_truncate(current_phys), page_size, free_in_pmm);
            current_virt += step;
            current_phys += step;
            unmapped_bytes += step;
        }
    ...
    pub unsafe fn map_page_ext(&self, virt: VirtAddr, phys: PhysAddr, page_size: &PageSize, flags: u64) -> bool {
        let indexes = PageIndexes::get_from_virt(virt);
        let pml4 = PageTable::from_cr3();
...
pub unsafe fn vmm_eba_map_page(virt: VirtAddr, phys: PhysAddr, page_size: &PageSize, alloc_in_pmm: bool) {
    PAGE_MANAGER.lock().eba_map_page(virt, phys, page_size, alloc_in_pmm);
}

pub unsafe fn vmm_eba_map_range(virt_start: VirtAddr, phys_start: PhysAddr, length: u64, page_size: &PageSize, alloc_in_pmm: bool) {
    PAGE_MANAGER.lock().eba_map_range(virt_start, phys_start, length, page_size, alloc_in_pmm);
}

pub unsafe fn vmm_early_unmap_page(virt: VirtAddr, phys: PhysAddr, page_size: &PageSize, free_in_pmm: bool) {
    PAGE_MANAGER.lock().early_unmap_page(virt, phys, page_size, free_in_pmm);
}

pub unsafe fn early_unmap_range(virt_start: VirtAddr, phys_start: PhysAddr, length: u64, page_size: &PageSize, free_in_pmm: bool) {
    PAGE_MANAGER.lock().early_unmap_range(virt_start, phys_start, length, page_size, free_in_pmm);
}

pub unsafe fn vmm_map_page(virt: VirtAddr, phys: PhysAddr, page_size: &PageSize) -> bool {
    vmm_map_page_ext(virt, phys, page_size, PageTableEntry::PRESENT | PageTableEntry::WRITABLE)
}

pub unsafe fn vmm_map_page_ext(virt: VirtAddr, phys: PhysAddr, page_size: &PageSize, flags: u64) -> bool {
    PAGE_MANAGER.lock().map_page_ext(virt, phys, page_size, flags)
}
...
pub unsafe fn vmm_unmap_range(virt_start: VirtAddr, length: u64, page_size: &PageSize) {
    PAGE_MANAGER.lock().unmap_range(virt_start, length, page_size);
}
